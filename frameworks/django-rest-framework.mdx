...
To secure your API views with API key authentication use the `HasAPIKey` permission class.
To specify required scopes, add the `required_scopes` attribute to your `APIView` class, or use the `@required_scopes` decorator if you use function-based views.
...
class ListItemsView(APIView):
    ...
    required_scopes = ["read:items"]  # Optional
...
from rest_framework.request import Request
from rest_framework.response import Response
from apitally.django_rest_framework import HasAPIKey, required_scopes
